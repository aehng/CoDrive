...
fun interface OpenRouterTransport {
    @Throws(IOException::class)
    fun post(requestBody: String, timeoutMillis: Int): Pair<Int, String>
}

class OpenRouterLlmClient @JvmOverloads constructor(
    private val apiKey: String,
    private val model: String,
    private val parser: GroqDecisionParser = GroqDecisionParser(),
    private val transport: OpenRouterTransport = defaultTransport(apiKey),
    ...
    private fun logRequestIfEnabled(requestBody: String) {
        if (BuildConfig.DEBUG) {
            try {
                Log.d("CoDrive.OpenRouter", "OpenRouter Request: " + requestBody)
            } catch (e: Exception) {
    ...
    private fun logResponseIfEnabled(statusCode: Int, responseBody: String) {
        if (BuildConfig.DEBUG) {
            try {
                Log.d("CoDrive.OpenRouter", "OpenRouter Response (" + statusCode + "): " + responseBody)
            } catch (e: Exception) {
                // ignore
            }
        }
    ...
    override fun infer(command: String, uiMap: PrunedUiMap): AgentDecision {
        if (apiKey.isBlank()) {
            return clarification("OpenRouter is not configured yet.")
        }
        ...
        while (attempt <= AgentPolicy.groqRetryAttempts) {
            ...
            try {
                ...
                return clarification("I could not reach OpenRouter right now.")
            }
        }
    ...
    private fun messageForHttpFailure(statusCode: Int, responseBody: String): String {
        ...
        return when (statusCode) {
            400 -> "OpenRouter rejected the request. Try a smaller command or verify model name. $detail"
            401, 403 -> "OpenRouter API key was rejected. Update the key in LLM Settings. $detail"
            404 -> "OpenRouter model or endpoint was not found. Check provider/model in LLM Settings. $detail"
            429 -> "I am rate-limited, retrying shortly. $detail"
            in 500..599 -> "OpenRouter is temporarily unavailable. Try again in a moment. $detail"
    ...
    companion object {
        ...
        fun defaultTransport(apiKey: String): OpenRouterTransport = OpenRouterTransport { requestBody, timeoutMillis ->
            val connection = (URL(OPENROUTER_URL).openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("HTTP-Referer", "https://codrive.local")
                setRequestProperty("X-Title", "CoDrive")
            }
